import { datVe } from "../actions/DatVeAction";
...
export const DatVeReducer = (state = DatVeState, action) => {
    switch (action.type) {
        case 'ADD_GHE_DUOC_CHON': {

            // UpdateGheDangChon = [...UpdateGheDangChon,action.value];
            state.arrGhe.forEach(DSghe => {
                DSghe.danhSachGhe.forEach(ghe => {
                    if (ghe.soGhe === action.value) {
                        let UpdateGheDangChon = [...state.arrGheDangChon, ghe];
                        state.arrGheDangChon = UpdateGheDangChon;
                    }
                })
            });
            console.log(state);
        ...
        case 'REMOVE_GHE_DUOC_CHON': {
            let UpdateGheDangChon = [...state.arrGheDangChon];
            state.arrGhe.forEach(DSghe => {
                DSghe.danhSachGhe.forEach(ghe => {
                    if (ghe.soGhe === action.value) {
                        const remove = UpdateGheDangChon.filter(UpdateGheDangChon => UpdateGheDangChon.soGhe !== action.value);
                        state.arrGheDangChon = remove;
                    }
                })
            });
            console.log(state);
        ...
        case 'DAT_VE':{
            let datVeArrGhe = [...state.arrGhe];
            state.arrGheDangChon.forEach(DSghe => {
                datVeArrGhe.forEach(ghe => {
                    ghe.danhSachGhe.forEach(DatVe =>{
                        if(DatVe.soGhe === DSghe.soGhe){
                            DatVe.daDat = true;
                        }
                    })
                })
            });
            state.arrGhe = datVeArrGhe;
            state.arrGheDangChon = [];
            return {...state};
        }
        default:
            return { ...state }
